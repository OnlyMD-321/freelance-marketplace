...
class ApplicationService {
  ...
  Future<Application?> applyForJob(String jobId) async {
    ...
    try {
      ...
      if (response.statusCode == 201) {
        ...
        print('Failed to apply for job: ${response.statusCode} ${response.body}');
  ...
  Future<List<Application>> listApplications({String? jobId}) async {
    ...
    final url = Uri.parse('$apiBaseUrl/applications').replace(queryParameters: queryParams);
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        print('Failed to list applications: ${response.statusCode} ${response.body}');
  ...
  Future<Application?> updateApplicationStatus(String applicationId, ApplicationStatus status) async {
     ...
     try {
       ...
       final String statusString = status.name[0].toUpperCase() + status.name.substring(1);
       // Or simply: final String statusString = status.name; if backend expects lowercase

       final body = jsonEncode({'status': statusString});

       final response = await http.patch(url, headers: headers, body: body); // Or PUT
       final responseData = jsonDecode(response.body);

       if (response.statusCode == 200) {
         return Application.fromJson(responseData['application']); // Assuming response is { message: '...', application: {...} }
       } else {
  ...
  // Future<bool> withdrawApplication(String applicationId) async { ... } // Likely a DELETE request
}